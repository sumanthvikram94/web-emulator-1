...
var authLogger = zluxUtil.loggers.authLogger;
  ...
  if (this.isLocalStorage()) {
    setInterval(function() {
      ...
      if (this.sessions.size > 10000) {
        authLogger.info('SessionStore.sessions.size=' + this.sessions.size);
      }
      const checkTimestamp = this.getTimestamp();
      this.sessions.forEach((session,key,map)=>
      {
        if (session.cookie && session.cookie.maxAge && checkTimestamp > session.lastTouch + session.cookie.originalMaxAge) {
          map.delete(key);
          //authLogger.debug('checkTimestamp=' + checkTimestamp +';session.lastTouch=' + session.lastTouch + '; session.cookie.originalMaxAge=' + session.cookie.originalMaxAge);
          //authLogger.debug('checkTimestamp time=' + Date(checkTimestamp).toString()+';session.lastTouch time=' + Date(session.lastTouch).toString() );
          //authLogger.info('Timeout cleaner expired session=' + JSON.stringify(session) + ' key=' + key + ' map.size=' + map.size);
        }
        if (checkTimestamp - session.lastTouch > defaultMaxAge2) {
          map.delete(key);
          //authLogger.debug('checkTimestamp time=' + Date(checkTimestamp).toString()+';session.lastTouch time=' + Date(session.lastTouch).toString() );
          //authLogger.debug('checkTimestamp=' + checkTimestamp +';session.lastTouch=' + session.lastTouch + '; defaultMaxAge2=' + defaultMaxAge2);
          //authLogger.info('Timeout cleaner dead session=' + JSON.stringify(session) + ' key=' + key + ' map.size=' + map.size);
        }
      });
    }.bind(this), 10000);
  }
...
SessionStore.prototype.addSession = function(sid, session) {
  if (!this.sessions.has(sid)) {
    ...
    //authLogger.info('session added, SessionStore count:'+this.sessionsQueue.length);
  }
  this.sessions.set(sid, session);
  // if(authLogger.parentLogger.configuration["_unp.auth"]>2){
  //   for(let value of this.sessions.values()){
  //     authLogger.debug('sessonStore.session detail:'+JSON.stringify(value));
  //   }
  // }
...
SessionStore.prototype.removeSid = function(sid) {
  if (this.sessions.delete(sid)) {
    ...
    //authLogger.info('session removed, count:'+this.sessions.size);
  }
}
