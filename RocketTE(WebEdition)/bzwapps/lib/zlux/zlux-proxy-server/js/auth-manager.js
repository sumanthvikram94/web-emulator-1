...
function AuthManager(options) {
  if (!AuthManager.prototype.isConfigValid(options.config)) {
    process.exitCode = UNP.UNP_EXIT_AUTH_ERROR;
...
AuthManager.prototype = {
  ...
  */
  validateAuthPluginList() {
    if (!this.authTypes) {
      bootstrapLogger.severe('The server found no authentication types. '
          + 'Verify that the server configuration file defines server authentication');
      process.exitCode = UNP.UNP_EXIT_AUTH_ERROR;
      throw new Error('Authentication validation error');
    }
