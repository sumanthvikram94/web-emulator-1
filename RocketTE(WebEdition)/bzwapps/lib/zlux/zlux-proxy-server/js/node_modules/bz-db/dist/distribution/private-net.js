...
import { TaskStatus } from '../workflow/index.js';
import { Utils, uid, elu, loggers, timeTracer } from '../services/index.js';
import { FileSyncMsgType, FileSync } from '../main/file-sync.js'; // BZ-15355, sync file for cluster
...
export var AUTO_SCALE_STATUS;
(function (AUTO_SCALE_STATUS) {
    AUTO_SCALE_STATUS[AUTO_SCALE_STATUS["NONE"] = 0] = "NONE";
    AUTO_SCALE_STATUS[AUTO_SCALE_STATUS["ACCEPTING"] = 1] = "ACCEPTING";
    AUTO_SCALE_STATUS[AUTO_SCALE_STATUS["ACCEPTED"] = 2] = "ACCEPTED"; // Auto scaling accepted and joint cluster
})(AUTO_SCALE_STATUS || (AUTO_SCALE_STATUS = {}));
    ...
    async checkPeerState() {
        ...
        if (peersRecord?.rowCount > 0) {
            ...
            for await (const peer of peersRecord.data) {
                if (peer.id !== selfId) {
                    let peerState = this.getPeerState(peer.id);
                    if (!peerState) {
...
class PrivateNet {
     ...
     * @param forceReLaunch
     */
    async launchNode(forceReLaunch = false) {
     ...
     * @returns
     */
    async sendTask(peer, channel, taskid, payload, isUnknownPeer = false, isNotZipped = false) {
        try {
            const data = {
    ...
    setTaskBroadcastStatus(task, peerId, status) {
        let broadcastStatus = task.getProperty('broadcast');
        if (!broadcastStatus) {
            broadcastStatus = new Map();
        }
     ...
     * @param payload
     */
    async broadcast(task, channel, payload, workflow, timeoutMs = 60000, toPeerIds) {
        ...
        if (peersRecord?.rowCount > 0) {
            if (toPeerIds) { //only send task to certain peers
                peersRecord.data = peersRecord.data.filter((p) => { return toPeerIds?.includes(p.id); });
            }
        ...
        if (this._nodeStateHandler.peers.size > 0) {
            for (let peerState of this._nodeStateHandler.peers.values()) {
                if (peerState.id === this._node.metadata.id)
                    continue;
                if (!peerMap[peerState.id])
                    continue;
                if (peerState.status !== NET_STATUS.PEER_CONNECTED) {
                    //TODO dial
    ...
    async autoScale(peerId) {
        return new Promise(async (resolve) => {
            try {
                ...
                const exist = metaPeers.data.findIndex((d) => d.id === peerId.toString()) > -1;
                if (exist) {
                ...
                asTask.execute();
                const asr = await asTask.getPromise(); // Gets the response from new instance
     ...
     * @returns
     */
    async checkStatus(peerInfo, isUnknownPeer = false, taskId) {
     ...
     * @returns
     */
    async checkFileSync4PullAll(peerId) {
        ...
        const checkTask = this.worker.workflow.generateTask({ desc: 'checkFileSyncDone4PullAll' }, () => {
            this._worker.statusService.status = STATUS.PULLING;
            const fsm = this._worker._fileSync.getFileSyncMsg4PullAll(checkTask.id);
     ...
     */
    async checkin(isMustDo = false, tskId) {
        ...
        if (isMustDo) {
            if (cit.length > 1) {
                return await cit[1].getPromise();
            }
        }
        else {
            if (cit.length > 0) {
                return await cit[0].getPromise();
            }
        }
    ...
    async createCheckinTask(taskId) {
        ...
        checkinProcess.onAsync('subTasksClose', async () => {
            ...
            const nodeData = checkinProcess.getProperty('nodeData');
            if (!nodeData) {
                if (this.isInCluster()) {
            ...
            for (const peerid of Array.from(nodeData.keys())) {
                ...
                const chains = nodeData.get(peerid).chainsStat;
                if (!chains) {
            ...
            if (correctPeerid !== '' && correctPeerid != undefined) { // Local chain is shorter than this "correctPeer"
                ...
                for (const peerid of Array.from(nodeData.keys())) {
                    const dbStat = nodeData.get(peerid).dbStat;
                    const chains = nodeData.get(peerid).chainsStat;
                    ...
                    for (const deName of Array.from(myDbStats.keys())) {
                        ...
                        const peerDeStat = dbStat?.get(deName);
                        const myDeStat = myDbStats.get(deName);
     ...
     */
    bookAutoCheckin(interval = CONFIG.AUTO_CHECKIN_INTERVAL) {
     ...
     * @returns
     */
    async autoResolveConflict(conflictEntity, parameters) {
    ...
    getPreferPeerId(parasObj, ownerId) {
        ...
        if (ownerId /* && this._peers.get(ownerId)?.status=== NET_STATUS.PEER_CONNECTED */)
            return ownerId;
        let correctPeerid = parasObj && parasObj.correctPeerid;
        if (correctPeerid && correctPeerid === this._node._nodeMeta.id)
            return correctPeerid; // self always running
        if (correctPeerid && this._nodeStateHandler.peers.get(correctPeerid)?.status === NET_STATUS.PEER_CONNECTED)
            return correctPeerid;
        const activePeers = this._nodeStateHandler.getActivePeersAndSelf();
     ...
     * @returns
     */
    async sendMsg(peer, channel, msg, isNotZipped = false) {
