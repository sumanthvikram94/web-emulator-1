...
import { AUTO_SCALE_STATUS } from './private-net.js';
...
import { ClusterNodeType } from '../main/metadata.js';
...
class NodeMsgHandler {
    ...
    setNodeEventHandlers() {
        this._node.addEventListener('peer:discovery', async (evt) => {
     ...
     */
    async parseMsg(buffList, connection, protocol) {
        ...
        if (b64 && b64.endsWith(CONFIG.PACKET_END_ESC)) {
            try {
                let msg = Buffer.from(b64.substring(0, b64.length - CONFIG.PACKET_END_ESC.length), 'base64');
    ...
    channelAuthCheck(channel, connection) {
     ...
     * @param handlerFunc
     */
    setMsgHandler(channel, handlerFunc) {
        const _this = this;
        this._node.handle(channel, ({ stream, connection }) => {
    ...
    async handleAutoScale(data) {
        ...
        try {
            ...
            const resp = {
                status,
                peerMeta: status ? this._pnNode.getPeerInfo() : undefined,
    ...
    async handleKick(data) {
        try {
            ...
            if (peerInfo.id === this._pnNode.getPeerInfo().id) {
                ...
                const others = peers.data.filter((d) => d.id !== peerInfo.id);
     ...
     * @param data
     */
    async handleFileSyncPullDone(data) {
        try {
            const { peer, taskid, payload } = data;
     ...
     * @param data
     */
    async handleForcePullDone(data) {
        try {
            const { peer, taskid, payload } = data;
     ...
     * @param data
     */
    async handleReqCheckinAck(data) {
        try {
            const { peer, taskid, payload } = data;
   ...
   * @param data
   */
    async handleSelectMemoryDone(values) {
                    ...
                    if (deMeta && deMeta.primaryKeys && deMeta.primaryKeys.length > 0
                        && (deMeta.fileName !== undefined // data entity with fixed filename, there is only 1 record 
                            ...
                            || (localChainStat.size * (1 - CONFIG.PULL_TOLERANCE_PERCENT) > chainStat.size
                                ...
                                && (localChainStat.txnSize - chainStat.txnSize) > (await this.worker.count(subject)).rowCount * CONFIG.PULL_TOLERANCE_PERCENT))) { // Do Full pull for the data entity
                        let data = await getData(deMeta, subject);
                        blocksMap.set(subject, { isFullPull: true, chain, data });
    ...
    async handleBatchTxn(data) {
        ...
        if (Array.isArray(payload)) {
            ...
            for (const data of payload) {
                ...
                if (subjectMap.has(subject))
                    continue; // In case there are multiple blocks for one data entity, we should validate the 1st block only
        ...
        let result = { isValid: true, inValidSubjects };
     ...
     * @param data
     */
    async handleBatchTxnConfirm(data) {
        const { peer, taskid, payload } = data;
    ...
    * @param data
    */
    async handleStatusConfirm(data) {
        const { peer, taskid, payload } = data;
     ...
     */
    sendFileToPeer(peerId, taskId, fsi) {
        return new Promise(async (resolve, reject) => {
            if (0)
                reject(`ignore error TS6133 ('reject' is declared but its value is never read)`);
     ...
     */
    async ensureSendStreamForPeer(peer, channel) {
     ...
     * @param msg
     */
    async sendMsgToPeer(peer, channel, msg, isNotZipped = false) {
        try {
