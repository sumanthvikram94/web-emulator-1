...
export interface FileSyncInfo {
  ...
  removed: boolean;
  timestamp?: number;
  _fsi_realSize4Check?: number;  // for pullAll
}


export enum FileSyncMsgType {
  FST_SEND_FILE = 1,
  FST_PULL_DONE
}
...
export interface FileSyncTask {
  peerId: string;
  fsm: FileSyncMsg;  // needed when type is FSM_SEND_FILE
}
...
export class FileSync {
  ...
  constructor(basePath: string, runTask: Function) {
    ...
    this._queue = new Queue(async(task: FileSyncTask, callback) => {
      await runTask(task);
      callback();
    }, { afterProcessDelay: 200 });  // a little delay
  }

  public pushSendTask(task: FileSyncTask) {
    Logger.logInfo(`FileSync::pushSendTask, type[${task.fsm.type}], taskId[${task.fsm.taskId}]`);
    this._queue.push(task);
  }

  public getFileSyncInfo(filePath: string, isSync4Add: boolean = true): FileSyncInfo {

    const fsi: FileSyncInfo  = {
      abs_path: filePath,
  ...
  public static isFileSyncMsg(obj: any): obj is FileSyncMsg {
    return ('type' in obj
      && 'taskId' in obj
      && 'error' in obj 
      && 'fsi' in obj 
  ...
  public static isFileSyncInfo(obj: any): obj is FileSyncInfo {
  ...
  public getFileSyncMsg4SendFile(fsi: FileSyncInfo, isPullAll: boolean = false): FileSyncMsg {
    const PREFIX_SIZE = 3;
    const chars = [..."ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789"];
    const prefix = [...Array(PREFIX_SIZE)].map(() => chars[Math.floor(Math.random()*chars.length)]).join('');
    const fsm: FileSyncMsg = {
    ...
    if (isPullAll) {
      const filePath: string = path.join(this._basePath, fsi.relative_path);
      fsm.fsi._fsi_realSize4Check = FileSync.getFileSize(filePath);  // for pullAll case
    }
    return fsm;
  }

  public getFileSyncMsg4PullAll(taskId: string): FileSyncMsg {
    const fsi: FileSyncInfo  = {
      abs_path: '',
      relative_path: '',
      size:  0,
  ...
  public static getRealFileState(baseDir: string, obj: any): string {
    if (!FileSync.isFileSyncInfo(obj)) {
      return '';
    }
    let prefix = '_fsi';
    const fsi: FileSyncInfo = obj as FileSyncInfo;
    if (fsi.removed) {
   ...
   * 65536 => "0000"
   */
  private numToHexString(val: number): string {
    if (val > 65535 || val < 0) {
      return "0000";
    } else {
      const pos: number = -1 * HEADER_SIZE;
      return (val + 0x10000).toString(16).substr(pos).toUpperCase();
    }
  }

  public deleteFile(val: FileSyncInfo) {    
    const filePath: string = path.join(this._basePath, val.relative_path);
    if (fse.existsSync(filePath)) {
      try {
  ...
  public getFileReadStream(taskId: string, val: FileSyncInfo) {
    let fileStream = null; 
    const fsiMsg: FileSyncMsg = {
      type: FileSyncMsgType.FST_SEND_FILE,
      taskId: taskId,
      error: '',
      fsi: val
    ...
    const str: string = JSON.stringify(fsiMsg);
    const data: string = '' + this.numToHexString(str.length) + str;    
    Logger.logInfo(`FileSync::getFileReadStream, send data is =${data}=`);

    const dataStream = Readable.from([data]);

    if (!fsiMsg.error && fileStream) {
      return mergeStream(dataStream, fileStream) as any;
    } else {
      return dataStream;
    }
  ...
  public async writeFileFromStream(source: any) {
    let header: string = '';
    let fsiData: string = '';
    let fsiDataSize: number = 0;
    let fsiMsg: FileSyncMsg = {} as FileSyncMsg;
    let bHasGotHeader: boolean = false;
    let bHasGotFsiData: boolean = false;
    let writeStream = null;
    const basePath: string = this._basePath;
    ...
    try {
      ...
      for await (const msg of source) {  // source might throw exception...
        ...
        if (!bHasGotHeader) {  // parse header to get data size
          const tmpSize: number = header.length;
          if ((tmpSize + msg.length) < HEADER_SIZE) {
            header += msg.slice().toString();
            continue;
          } else {
            bHasGotHeader = true;
            const endPos: number = HEADER_SIZE - tmpSize;
        ...
        if (!bHasGotFsiData) {  // parse FileSyncInfo
          const tmpSize: number = fsiData.length;
          if ((tmpSize + msg.length - startPos) < fsiDataSize) {
            fsiData += msg.slice(startPos).toString();
            continue;
          } else {
            bHasGotFsiData = true;
            const endPos: number = startPos + fsiDataSize - tmpSize;
            ...
            try {
              fsiMsg = JSON.parse(fsiData) as FileSyncMsg;
              if (fsiMsg.error) {
                ...
                const filePath: string = path.join(basePath, fsiMsg.fsi.relative_path);
