...
import { DataEntityMetadata, PersistMethod, PersistType, SelectConstraints,
    DatabaseMetadata, DataStrategy,TxnResult,SchemaType, AnalyzeMethod, TxnOps } from '../main/metadata.js'; // ProcessPooling, 
...
import { Utils, Encryption, FileWatcher, yaml, loggers, ILoggerConfig, LoggerConfigurer, ILogger, Logger } from '../services/index.js'
...
// const _flagWatchFile = Symbol();

interface FsQueueOptions {
    type: string
    option: any
    txnId?: string
}

class BlueZoneDatabase implements InternalDatabaseInterface{
    ...
    [_execQueue]: Queue;
    // [_processPool]: any;
    // [_processPartitionMap]: Map<number, number[]>;
    // [_isPooledProcess]: boolean;
    [_databaseMetadata]: DatabaseMetadata;
    [_dataEntityMetadata]: Map<string, DataEntityMetadata>;
    [_basePath]: string;
    // [_onLoadReady]: (results:any[])=>void;
    ...
    public name = 'MainProcess';

    constructor(databaseMetadata: DatabaseMetadata, processName?:string){ // , isPooledProcess?: boolean
        ...
        this[_promLoadReady] = new Promise<any>( (resolve) => {
     ...
     */
    private _createQueue(): void {
        this[_queue] = new Queue(async (options: any, cb: any)=> {
            ...
            if (node.type === 'add') {
                ...
                if(metadata && (!metadata.persistType || metadata.persistType && (metadata.persistType as PersistType) === PersistType.PERSIST_TYPE_OBJECT)
                    ...
                    try{
                        if(metadata && metadata.persistMethod && (metadata.persistMethod as PersistMethod) === PersistMethod.PERSIST_METHOD_COMBINED_FILE  ){
     ...
     * @param isIni 
     */
    private _addFile(option: any, txnId: string): Promise<any> {
        return new Promise<any>((resolve, reject) => {
            try {
                ...
                if(str.length>0){
                    ...
                    fse.outputFile(option.path,str,(err: any) => {
                        if (err){
     ...
     */
    private _deleteFile(path: string, txnId: string): Promise<any> {
        return new Promise<any>((resolve, reject)=>{
     ...
     * @param path dir path
     */
    private _deleteDir(path: string, txnId: string){
        return new Promise<any>((resolve,reject) => {
     ...
     * Creates the queue for file system operation
     */
    private _createExecQueue(): void {
        const that = this;
        this[_execQueue] = new Queue((options: any, cb: any)=> {
     ...
     */
    private _execInQueue(option:any): Promise<any> {
        return new Promise<any>((resolve, reject) => {
            const txnId = option.options?.txnId
            const queueDepth = (this[_execQueue] as any).length
            if (queueDepth > 1) {
            ...
            this[_execQueue].push(option, (err: any, result: any)=>{
     ...
     * @param option : Object includes {type: add|delete|sync|deleteDir, option}
     */
    private _addQueue(option: FsQueueOptions): Promise<any> {
        const that = this;
        return new Promise<any>((resolve, reject) => {
            const queueDepth = (this[_queue] as any).length
            if (queueDepth > 1) {
     ...
     */
    private _createDirsSync(dirpath: string) {
     ...
     * @param log 
     */
    private _logInfo(log: string){
        const logger = this[_logger];
        logger.logInfo(log);
    }

    private _logWarn(log: string){
        const logger = this[_logger];
        logger.logWarn(log);
    }

    private _logSevere(log: string){
        const logger = this[_logger];
        logger.logSevere(log);
    }

    private _logDebug(log: string){
        const logger = this[_logger];
        logger.logDebug(log);
    }

    private _getStoragePath(metadata: DataEntityMetadata): string {
        const subPath = metadata.filePath? metadata.filePath: metadata.name;
        return path.join(this[_basePath], subPath);
    }
    
    private _getStorageFullPath(subPath:string): string {
        return path.join(this[_basePath], subPath);
    }

    private _loadDataEntitys(): Promise<any>[]{
        this._logDebug(`Starting Rocket Bluezone Database: ${this.name}`);
        const loadStatuses: Promise<any>[] = [];
    ...
    private _initDataObject(metadata: any): any{
        const indexes: any = {};
        if (metadata.indexes) {
    ...
    private _deleteKeyValueFromObj(obj: any, keyPath:string): void {
        const keyPathArray = keyPath.split('.');
        let tmpObj = obj;
        for (let i=0; i< keyPathArray.length - 1; i++){
            tmpObj = tmpObj[keyPathArray[i]];
        }
        if (tmpObj[keyPathArray[keyPathArray.length - 1]]) delete tmpObj[keyPathArray[keyPathArray.length - 1]];
    }
     ...
     */
    private _getDeepKeyValueStr(deObj: any, keyPath:string): string|null {
        const keyPathArray = keyPath.split('.');
        let obj = deObj;
    ...
    private _setDeepKeyValueStr(deObj: any, keyPath:string, data:string) {
    ...
    private _deletePKValue(value: Object, metadata: DataEntityMetadata): void{
    ...
    private _getPKValue(value: Object, metadata: DataEntityMetadata): string{
        let combPkValue = this._getCombFieldsValue(value, metadata.primaryKeys);
        return combPkValue;
    }

    private _getCombFieldsValue(value: Object, fields: string[]): string{
        let combValue = '';
        if (fields){
    ...
    private _loadPartitions(metadata: DataEntityMetadata, partitions: Map<string, any>): any{
        // Init data entity object
        const deObj:any = this._initDataObject(metadata);
        this._createDirsSync(deObj.storagePath);
        const partMap: Map<string, any> = new Map()
        deObj['partitions'] = partMap;
        if (metadata.primaryKeys){
            partitions.forEach((partition: any, key: string) => {
                if (partition){
            ...
            deObj.values.forEach((value: any) => {
    ...
    private _buildIndexes(dataEntity: any, value: any): void {
    ...
    private _loadValues(metadata: DataEntityMetadata, values: any[]): any {
        // Init data entity object
        const deObj:any = this._initDataObject(metadata);
        this._createDirsSync(deObj.storagePath);
    ...
    _replaceDE(dataEntityName: string, newDataEntity: DataEntity){
     ...
     */
    private _manage(metadata: DataEntityMetadata, isRefresh?: boolean, partitions?: string[]): Promise<any>{
        return new Promise((resolve) => {
            ...
            this._persistRead(metadata, partitions).then((values: Map<string, any>) => {
                ...
                if (partitions && partitions.length > 0) dataEntities[dataEntityName]['managedPartitions'] = partitions;
                if (isRefresh) {
    ...
    private _getMatchedIndex(metadata: DataEntityMetadata, filter: any): string|null{
        if (! metadata.indexes ){
            return null;
        }
     ...
     */
    private _arrayIncludes( parentArray: string[], childArray: string[]): boolean{
    ...
    private _arrayMinus(arr1: any[], arr2: any[]){
        return arr1.filter( ele => !arr2.includes(ele));
    }

    private _searchDeepKeysArr(valueArr: any[], filter: any, constraints?: SelectConstraints): any[]{
        if (!filter){
            return valueArr
        }
        const filterKeys = Object.keys(filter);
    ...
    private _getSubObj(dataObj: any, keys: string[]){
        const subObj:any = {};
        keys.forEach( key => {
    ...
    */
    private _filter(dataEntity: any, filter: any, constraints?: SelectConstraints): any[] | null{
     ...
     */
    private _filterByOneRecord(dataEntity: any, filter: any, constraints?: SelectConstraints): any[] | null{
        const filterKeys = Object.keys(filter);
        ...
        if(isPersistListFile && isPersistListFileArr) {
            ...
            keys.forEach((k, i) => {
                result = _.clone(result.filter((d: any) => d[k] === values[i]) || [])
            })
    ...
    private _deleteAll(dataEntity: any, options?: TxnOps): Promise<TxnResult>{
        return new Promise<TxnResult>((resolve) => {
            const txnId = options?.txnId
            txnLogger.logDebug(`${this.name} delete all`, txnId);
            if ((dataEntity.metadata.persistMethod as PersistMethod) === PersistMethod.PERSIST_METHOD_ONLY_MEMORY) {
     ...
     * @param pkVal 
     */
    private _deleteDKVByPk(dataEntity: any, pkVal: string):void{
    ...
    private _deleteFromIndexes(dataEntity: any, value: any): void{
        const metadata = dataEntity.metadata
        if (!metadata.indexes) return;
        const idxNames = Object.keys(metadata.indexes);
        idxNames.forEach((idxName: string)=> {
    ...
    private _persistDelete(dataEntity: any, deleteValues: any[], filter?:Object, txnId?: string): Promise<any>{
        return new Promise<any>(async (resolve) => {
            let persistMethod = dataEntity.metadata.persistMethod;
            
            if (persistMethod && (persistMethod as PersistMethod) === PersistMethod.PERSIST_METHOD_LIST_FILE){
                const proms: Promise<any>[] = [];
                let persistType = dataEntity.metadata.persistType;
                if(persistType && (persistType as PersistType) === PersistType.PERSIST_TYPE_ARRAY) {
                    const key = dataEntity.metadata.primaryKeys[0] ;
                    // const dkey = Object.keys(dataEntity.data)[0] || '___NULL___';

                    deleteValues.forEach(val => {
                        const vIndex = dataEntity.values[0].findIndex((d:any) => d[key] === val[key]);
                    ...
                    deleteValues.forEach((value: Object) => {
                ...
                if (metadata.dataStrategy === DataStrategy.NO_CACHE){ // No cache data, use filter to delete
                    ...
                    for await (const partitionNum of partitionNums){
                        ...
                        if (_.isEmpty(partData)) continue
                        if (this._isAllPKColIncluded(filter, metadata)){ // deleting with PK
                            ...
                            tempFilters.forEach((fl) => {
                                ...
                                if (!pkVal || pkVal.includes('___NULL___')) return // the filter pk not exists
                            ...
                            const tempFilters = Utils.splitObject(filter)
                            tempFilters.forEach( (fl) => {
                                const rows: any[] = _.filter(Object.values(partData), fl)
    ...
    private _deleteByFilter(dataEntity: any, filter: any, options?: TxnOps): Promise<any>{
        return new Promise<any>(async (resolve)=>{
            const txnId = options?.txnId
            let deleteArr = []
            if (dataEntity.metadata.dataStrategy !== DataStrategy.NO_CACHE){
                ...
                if ((dataEntity.metadata.persistMethod as PersistMethod) === PersistMethod.PERSIST_METHOD_ONLY_MEMORY) {
                    ...
                    const result: TxnResult = {status: true, results, message: ''}
            ...
            } else if (dataEntity.metadata.persistMethod && (dataEntity.metadata.persistMethod as PersistMethod) === PersistMethod.PERSIST_METHOD_LIST_FILE && 
    ...
    private _getPathFromPK(dataEntity: any, pkValue: string): string{
        return path.join(dataEntity.storagePath, this._getFileNameFromMeta(dataEntity.metadata, pkValue));
    }

    private _getFileNameFromMeta(metadata: DataEntityMetadata, pkValue: string): string{
    ...
    private _insertIndexes(dataEntity: any, value: any): void{
        if (!dataEntity.metadata.indexes) {
            return;
        }
     ...
     * @param value 
     */
    private _constraintIgnoreCaseFields(ignoreCaseFields: string[], value: any): void {
        if (ignoreCaseFields) {
    ...
    private _insert(dataEntity: any, value: any, options?: TxnOps): Promise<any>{
        return new Promise<any>(async (resolve) => {
            const metadata = dataEntity.metadata
            ...
            if (persistMethod === PersistMethod.PERSIST_METHOD_ONLY_MEMORY) {
                const result: TxnResult = {status: true}
                try{
            ...
            this._persistSave(dataEntity, value, options?.txnId).then(async (result) => {
                ...
                if (metadata.dataStrategy !== DataStrategy.NO_CACHE) {
                    dataEntity.keys.push(pkVal);
                    if (persistMethod && (persistMethod as PersistMethod) === PersistMethod.PERSIST_METHOD_LIST_FILE &&
                    persistType && (persistType as PersistType) === PersistType.PERSIST_TYPE_ARRAY && value != null) {
                        ...
                        vals.forEach((v: any) => {
                            const vIndex = dataEntity.values[0].findIndex((d: any) => d[key] === v[key]);
                            ...
                            const dIndex = dataEntity.data[dkey].findIndex((d: any) => d[key] === v[key]);
    ...
    private _isAnyPKColIncluded(value: any, metadata: DataEntityMetadata): boolean{
    ...
    private _isAllPKColIncluded(value: any, metadata: DataEntityMetadata): boolean{
        const pkCols = metadata.primaryKeys;
    ...
    private _genStrHashcode(str: string): number{
        let hash = 0;
    ...
    private _genPersistFileObj(): any{
        const obj: any = {};
        return obj;
    }
    ...
    private _handleFileChange(dirPath: string, metadata: DataEntityMetadata, stats: fs.Stats, filename?: string, event?: string){
     ...
     */
    private _watchPath(filePath : string, metadata: DataEntityMetadata, isWatchParent = true):void {
        ...
        if (this[_dataStrategy] === DataStrategy.FILE_WATCH && metadata.dataStrategy !== DataStrategy.NO_CACHE && !metadata.isInternal){
            try{
                ...
                if (this[_watchedPaths].has(metadata.name)) return; // Avoid watching the same path for multiple times.
                ...
                watcher.on('change', (file: string, stats: fs.Stats) => {
    ...
    private _persistRead(metadata: DataEntityMetadata, partitions?: any[],filter?:Object): Promise<Map<string, any>>{
        return new Promise<Map<string, any>>((resolve) => {
            const partsMap: Map<string, any> = new Map()
            ...
            if (persistMethod && (persistMethod as PersistMethod) === PersistMethod.PERSIST_METHOD_LIST_FILE){
                ...
                if(!fileName){ //mutiple file read by directory path
                    ...
                    this[_util].getDataMap(filePath, fileFilter, this[_dataStrategy] === DataStrategy.FILE_WATCH).then((dataMap: Map<string, any>) => {
                    ...
                    this._readSignleFile(mergedMetadata).then((values: any) => {
                ...
                filterPartitions.forEach((partNum) => {
                    ...
                    const prom = this[_util].readFilePromise(filePath,'utf8', this[_dataStrategy] === DataStrategy.FILE_WATCH).then((partition: any) => {
                ...
                Promise.all(promises).then((parts:[any[]]) => {
    ...
    private _calSinglePartNum(metadata: DataEntityMetadata, partCol: string, row: any){
    ...
    private _calAllPartNums(metadata: DataEntityMetadata, row: any){
    ...
    private _calFilterParitions(metadata: DataEntityMetadata, partFilter?:any, filterPartitions?: any[]): string[]{
        let parts: string[] = []
    ...
    private _persistReadNoPKData(metadata: DataEntityMetadata): Promise<any[]>{
        return new Promise<any[]>((resolve) => {
            let persistMethod = metadata.persistMethod;
            if (persistMethod && (persistMethod as PersistMethod) === PersistMethod.PERSIST_METHOD_LIST_FILE){
                let fileName=metadata.fileName || ''
                ...
                if(!fileName && isNoPK){ // is NO PK and no fileName, need return fileName.
                    ...
                    const filePath = this._getStoragePath(metadata);
                    this._createDirsSync(filePath); // create the path in case it doesn't exist
                    this[_util].returnDataWithName(filePath, fileFilter, metadata.persistType , metadata.isAllowSubFolder).then((values: any) => {
                        resolve(values);   
                    }).catch((err: any) => {
                        resolve(err);
                    });
                }else{ 
                     this._persistRead(metadata).then((value: Map<string, any>) => {
    ...
    private  _readSignleFile(metadata: DataEntityMetadata): Promise<any>{
    ...
    _mergeFilterIntoMetaData(metadata:DataEntityMetadata,filter?:Object){
    ...
    private _persistSave(dataEntity: any, value: any, txnId?: string): Promise<any>{
        return new Promise<any>( async (resolve,reject) => {
            const metadata = dataEntity.metadata
            const persistMethod = metadata.persistMethod;
            const persistType = metadata.persistType;
            const pkVal = this._getPKValue(value, dataEntity.metadata);
            const inMemory = (persistMethod as PersistMethod) === PersistMethod.PERSIST_METHOD_ONLY_MEMORY;

            if(inMemory) {
                resolve({status: true, message: "Saved successfully in only memory mode"});
            } else if (persistMethod && ((persistMethod as PersistMethod) === PersistMethod.PERSIST_METHOD_LIST_FILE)){
                let option = {
                    data:value,
                    path:'',
                    metadata
                ...
                if(!!pkVal || metadata.fileName){ //exist PK or metaData.fileName
                    ...
                    Object.defineProperty(option, 'doFormat', {   //format the json file
                        value: true,
                        writable: false
                      });
                }
                if(persistType && (persistType as PersistType) === PersistType.PERSIST_TYPE_ARRAY) {
                    ...
                    let newValue: Array<any> = _.clone(values);
                    newValue = [].concat(...newValue); // convert to linear array

                    if(Array.isArray(value)) {
                        newValue = _.concat(newValue, value)
                    } else {
                        const index = dataEntity.keys.findIndex((d: any) => d === pkVal);
                        if(index > -1) {
                ...
                }else if(persistType && (persistType as PersistType) === PersistType.PERSIST_TYPE_RAW) {  //upload, buffer format
                    let fileObj=value;
                    // BZ-15480, support single raw file entity 
                ...
                if (metadata.dataStrategy === DataStrategy.NO_CACHE){
                    ...
                    if (!dataEntity['partitions']){
                        const emptyMap: Map<string, any> = new Map()
    ...
    private _bulkLoadPersistPartition(dataEntity: any, values: any[], options?: TxnOps): Promise<any>{
        const txnId = options?.txnId
        ...
        return new Promise<any> (async resolve => {
            ...
            const metadata = dataEntity.metadata
            const promsPartition: Promise<any>[] = [];
            const promsData: any[] = [];
            const partitionValues: Map<any, any[]> = new Map();
            const clonePartitions:any = {};
    ...
    private _bulkLoadPersistFileList(dataEntity: any, values: any[], options?: TxnOps): Promise<any>{
        return new Promise<any> (resolve => {
            const proms: Promise<any>[] = [];
            const metadata = dataEntity.metadata
            const constraint = options?.bulkloadConstraints
            if (constraint && constraint.isReplaceAsUpdate()){
                ...
                values.forEach(value => {
                    const filter:any = {};
                    metadata.primaryKeys.forEach((keyCol:string) => {
    ...
    private async _bulkLoadOnlyMemory(dataEntity: any, values: any[], options?: TxnOps): Promise<TxnResult> {
        const results: TxnResult[] = []
        const result: TxnResult = {status: true, results, message: ''}
        const constraint = options?.bulkloadConstraints
        values.forEach((value) => {
    ...
    private _bulkLoad(dataEntity: any, values: any[], options?: TxnOps): Promise<TxnResult>{
        return new Promise<TxnResult> (async (resolve) => {
            const txnId = options?.txnId
            let persistMethod = dataEntity.metadata.persistMethod;
            if ((dataEntity.metadata.persistMethod as PersistMethod) === PersistMethod.PERSIST_METHOD_ONLY_MEMORY) {
    ...
    private async _isLockExist(dataEntityName: string, filePath: string): Promise<boolean> {
        let existLock
        try{
    ...
    async _obtainResourceLock(dataEntityName: string, times = 0): Promise<any> {
        const funcNextRound = (resolve: Function, reject: Function) => {
            if ( times % 10 === 0 ){
    ...
    async _releaseResourceLock(dataEntityName: string): Promise<any>{
        return new Promise((resolve) => {
            const dataEntity = this[_dataEntities][dataEntityName]
            if (this[_databaseMetadata].getDataStrategy() !== DataStrategy.FILE_WATCH || (dataEntity.metadata.persistMethod as PersistMethod) === PersistMethod.PERSIST_METHOD_ONLY_MEMORY) { 
    ...
    private _updateOrInsert(dataEntity: any, value: any, options?: TxnOps): Promise<TxnResult>{
        return new Promise<any>(async (resolve) => {
            const txnId = options?.txnId
            const filter: any = {};
            const metadata = dataEntity.metadata
            metadata.primaryKeys.forEach( (key: string) => {
    ...
    private _update(dataEntity: any, updateValues: any[], value: any, options?: TxnOps): Promise<TxnResult>{
        return new Promise<any>(async (resolve) => {
            let persistMethod = dataEntity.metadata.persistMethod;
            const txnId = options?.txnId
            try {
            ...
            if ((dataEntity.metadata.persistMethod as PersistMethod) === PersistMethod.PERSIST_METHOD_ONLY_MEMORY) {
                ...
                const partitionValues: Map<any, any[]> = new Map();
                const partitionValuesDelete:  Map<any, any[]> = new Map();
                const clonePartitions:any = {}; // the data to write file
                const metadata = dataEntity.metadata
                ...
                const proms: Promise<any> [] = [];
                // To improve performance, all updates in 1 partition will only persist for once.
                Object.keys(clonePartitions).forEach((partNum) => { // Write the partition files
                    const filePath = this._getPathFromPK(dataEntity, partNum + '');
                    const option = {
                        data: clonePartitions[partNum],
                        path: filePath,
                ...
                Promise.all(proms).then( async (results) => { // updates the partition data in memory
                    if (metadata.dataStrategy !== DataStrategy.NO_CACHE){
                        results.forEach((result: any) => {
                            if (result && result.status){
                                const partNum = result.partition;
     ...
     */
    public  selectSync(dataEntityName: string, filter?: Object, options?: Object): any{
        // TBD, should check cache for frequently queried data.
        const dataEntities = this[_dataEntities];
        const metadata:any = this[_dataEntityMetadata].get(dataEntityName);
        ...
        const de: DataEntity = dataEntities[dataEntityName];
     ...
     */
    public select (dataEntityName: string, filter?: Object, options?: Object): Promise<any>{
        return new Promise<any>((resolve)=>{
            this._statusReady().then(async () => { // Suspends the select if there is data refresh.
                const metadata:any = this[_dataEntityMetadata].get(dataEntityName);
                let returnObj:any = {};
                if (!metadata){
                ...
                if (metadata.dataStrategy === DataStrategy.NO_CACHE) {
                    ...
                    if(values && Array.isArray(values)){
                        ...
                        if (!metadata.persistMethod || metadata.persistMethod === PersistMethod.PERSIST_METHOD_COMBINED_FILE ){
                            ...
                            const tempPartitions:any = {}
                            values.forEach((partObj) => { // each partition
                                if (_.isEmpty(partObj)) return
     ...
     * 
     */
    public selectNoPKData (dataEntityName: string): Promise<any>{
        return new Promise<any>((resolve)=>{
            this._statusReady().then(async () => { // Suspends the select if there is data refresh.
                const metadata:any = this[_dataEntityMetadata].get(dataEntityName);
                let returnObj={};
     ...
     * @returns 
     */
    public async partitions(dataEntityName: string): Promise<DEPartitions> {
        return new Promise<any>((resolve)=>{
            this._statusReady().then(async () => {
                const metadata:DataEntityMetadata | undefined = this[_dataEntityMetadata].get(dataEntityName);
                const de: DataEntity = this[_dataEntities][dataEntityName];
                if (!metadata){
                    return resolve({status: false, message: MSG_DATA_ENTITY_NOT_EXIST});
                }
                if (metadata.dataStrategy === DataStrategy.NO_CACHE) {
    ...
    public writePartitions(parts: DEPartitions): Promise<any>{
        return new Promise<any>((resolve) => {
            const execOptions = {
     ...
     * @returns 
     */
     private async _doWritePartitions(parts: DEPartitions): Promise<any> {
        return new Promise<any>((resolve)=>{
            this._statusReady().then(async () => {
    ...
    public delete(dataEntityName: string, filter?: any, options?: TxnOps): Promise<any>{
        return new Promise<any>((resolve) => {
            const execOptions = {
                action: 'delete',
                dataEntityName: dataEntityName,
     ...
     */
    private _statusReady(): Promise<any> {
        const that = this;
        return new Promise<any>((resolve) => {
    ...
    private _execDelete(dataEntityName: string, filter: any, options?: TxnOps): Promise<any>{
        return new Promise<any>((resolve) => {
            this._statusReady().then( async ()=>{
                const txnId = options?.txnId
                if (!options || !options.isSilent) {
                ...
                // } else {
                    ...
                    try{	
                        ...
                        if (filterKeys.length === 0){
                            this._deleteAll(de, options).then(async (result:any)=>{
                                await this._releaseResourceLock(metadata.name)
                                resolve(result);
                                // de.isLocked = false;
                            }).catch(async err => {
                        ...
                        this._deleteByFilter(de, filter, options).then(async (result:any)=>{
                            await this._releaseResourceLock(metadata.name)
                            resolve(result);
                            // de.isLocked = false;
                        }).catch(async (err) => {
     ...
     * @param value 
     */
    public insert(dataEntityName: string, value: any, options?: TxnOps): Promise<any>{
        return new Promise<any>((resolve) => {
            const execOptions = {
                action: 'insert',
                dataEntityName: dataEntityName,
     ...
     * @param value 
     */
    private _execInsert(dataEntityName: string, value: any, options?: TxnOps): Promise<any>{
        return new Promise<any>((resolve)=>{
     ...
     * @param constraints 
     */
    public update(dataEntityName: string, filter:any, value: any, options?: TxnOps): Promise<any>{
        return new Promise<any>((resolve) => {
            const execOptions = {
                action: 'update',
                dataEntityName: dataEntityName,
    ...
    private _execUpdate(dataEntityName: string, filter:any, value: any, options?: TxnOps): Promise<any>{
        return new Promise<any>((resolve)=> {
            this._statusReady().then( async ()=>{
                const txnId = options?.txnId
                if (!options || !options.isSilent) {
     ...
     * @param value 
     */
    public updateOrInsert(dataEntityName: any, value: any, options?: TxnOps): Promise<any>{
        return new Promise<any>((resolve) => {
            const execOptions = {
    ...
    private _execUpdateOrInsert(dataEntityName: any, value: any, options?: TxnOps): Promise<any>{
        return new Promise<any>((resolve)=> {
            this._statusReady().then( ()=>{
                const txnId = options?.txnId
    ...
    public bulkLoad(dataEntityName: string, values: any[], options?: TxnOps): Promise<any>{
        return new Promise<any>((resolve) => {
            const execOptions = {
                action: 'bulkLoad',
                dataEntityName: dataEntityName,
    ...
    private _execBulkLoad(dataEntityName: string, values: any[], options?: TxnOps): Promise<any> {
        return new Promise<any>((resolve)=> {
            this._statusReady().then( ()=>{
                const txnId = options?.txnId
                if (!options || !options.isSilent) {
     ...
     * @param constraint 
     */
    private _execWatchEvent(dataEntityName: string): Promise<any> {
        return new Promise<any>((resolve)=> {
     ...
     * @param dataEntityName 
     */
    public refreshDataEntity(dataEntityName: string): Promise<any>{
        return new Promise<any>((resolve)=> {
            this[_promLoadReady].then(()=>{
                const metadata: any = this[_dataEntityMetadata].get(dataEntityName);
                if (!metadata){
    ...
    public waitLoadReady(): Promise<any>{
        return new Promise<any> ((resolve) => {
            this[_promLoadReady].then(()=>{
                resolve(true); 
            })
    ...
    public changeLogger(config: ILoggerConfig): void {
    ...
    private _selectFromCache(dataEntityName: string, filter?: Object, options?: Object){
        const opt = new SelectConstraints(options);
        const metadata:any = this[_dataEntityMetadata].get(dataEntityName);
        // const isPersistListFile = metadata.persistMethod && metadata.persistMethod === PersistMethod.PERSIST_METHOD_LIST_FILE;
        // const isOnlyMemory = metadata.persistMethod && metadata.persistMethod === PersistMethod.PERSIST_METHOD_ONLY_MEMORY;
     ...
     * @returns 
     */
    public async analyzeDataEntity(dataEntityName: string, isIncludeRows = true, isDoHash = false): Promise<DEStatistics>{
        const metadata:any = this[_dataEntityMetadata].get(dataEntityName);
        if (!metadata) {
            return {
                name: dataEntityName
        ...
        if (isNoCache){
            if (metadata.analyzeMethod === AnalyzeMethod.ANALYZE_METHOD_FILE_SIZE) {
                ...
                if (isIncludeRows){
                    ...
                    statistics.rowStats = rowStats.sort((a:any, b: any) => {
                        return a[0] > b[0]?1:-1
                    })
     ...
     * @returns 
     */
    public async count(dataEntityName: string): Promise<any>{
        const metadata:any = this[_dataEntityMetadata].get(dataEntityName);
        if (!metadata){
    ...
    private async _appendExtendEntity(){
    ...
    private async _loadEntity(dMetaData:DataEntityMetadata): Promise<any>{
    ...
    private _releaseEntity(entityName:string){
    ...
    public async create(type:SchemaType,value:object, options?: TxnOps): Promise<any>{
        ...
        if(!this[_dataEntityMetadata].has(dMetaData.name)){ //exist, can not create again
            if(type===SchemaType.PERSIST_ENTITY){
                ...
                return result;
            }else if(type===SchemaType.TEMP_ENTITY){
    ...
    public async drop(type:SchemaType, entityName:string, options?: TxnOps){
