...
import { Transaction, TxnData } from './transaction.js'
...
class Block implements BlockData{
  ...
  public isConflictWithApproved = false
  ...
  constructor(data: BlockData) {
    ...
    if (data.transactions.length){
      for (let txnJson of data.transactions){
        ...
        if (!txn.isValid()) throw new TransactionError(`Invalid transaction ant-Block creation: ${txnJson.toString()}`);
  ...
  setRedundentBlocks(blocks: BlockData[]){
  ...
  async executeTxns(db: InternalDatabaseInterface){
    const blockData = prvProps.get(this)
    const results = []
