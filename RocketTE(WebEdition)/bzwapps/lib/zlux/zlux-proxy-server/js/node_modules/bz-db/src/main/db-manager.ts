...
import { DbWorker, startWorker } from './db-worker.js'
import { ILoggerConfig, uid, Utils, Logger } from '../services/index.js'
import { eventHandler } from '../event/event-handler.js'
import { FileSyncInfo } from './file-sync.js'  // BZ-15355
...
class DbManager implements DatabaseInterface{
    ...
    async startWorker(){
        ...
        const observer: Function = async (taskData: any) => {
                        ...
                        if(cmd === STATUS.DATA_CONFLICT){
                            if(eventHandler.occuredEventMap[cmd].length === 0 || 
                                (eventHandler.occuredEventMap[cmd].length > 0 && 
                                    eventHandler.occuredEventMap[cmd].filter((e: any) => (e.parames.dataEntity === parames.dataEntity && e.parames.type === parames.type && e.parames.remotePeerId === parames.remotePeerId)).length === 0)){
        ...
        if (this.metadata.isWorkerThreadEnabled()){ // run db-worker in seperate thread
            this._worker = await spawn<DbWorker>(new Worker('./db-worker.js'), {timeout: 60000}) // Wait 60 seconds for the worker creation.
    ...
    async insert(dataEntityName: string, value: any, isSilent = false){
    ...
    async delete(dataEntityName: string, filter?: any, constraints?: SelectConstraints, isSilent = false){
    ...
    async introduceNode(nodeInfo: IPeerMeta){
    ...
    async batchTxn(batchTxnData: BatchTxnData[]){
        return await this._worker.batchTxn(batchTxnData)
        
    }

    async kickNode(nodeInfo: IPeerMeta){
        return await this._worker.kickNode(nodeInfo)
    }

    async select(dataEntityName: string, filter?: Object, options?: Object){
        return await this._worker.select(dataEntityName, filter, options)
    }

    async partitions(dataEntityName: string){
        return await this._worker.partitions(dataEntityName)
    }
    
    async writePartitions(parts: DEPartitions){
    ...
    async waitLoadReady(){
        if (this._worker && this._worker.waitLoadReady){
            ...
            return new Promise((resolve, reject) => {
                if (this._times % 5 === 0) console.warn('Awaiting DB worker creation, seconds: ' + this._times)
                if ( ++ this._times > 120) reject('Database loading timeout')
                else {
                    setTimeout(() => {
    ...
    async forcePullData(peerId: string){
        return await this._worker.forcePull(peerId)
    }

    async pushToPeers(entities: Array<any>, checkInFlag: boolean = false){
        await this._worker.pushToPeers(entities, checkInFlag)
    }

    async resolvePeers(peerId: string){
    ...
    async getFileSyncInfo(filePath: string, isSync4Add: boolean = true): Promise<FileSyncInfo> {  // BZ-15355
    ...
    async getUID(radix?:number, level?:number){
        return await this._worker.getUID(radix, level)
    }

    getUIDSync(radix?:number, level?:number){
    ...
    async exec(cmd: string, parames?: any[], peerId?: string){
        if (this.metadata.isWorkerThreadEnabled()) {
            if (peerId) {
    ...
    async create(schemType:string,dMetaDataObj:object){
        return await this._worker.create(schemType,dMetaDataObj)
    }

    async drop(schemType:string,entiryName:string){
                ...
                //     console.log(VERSION)
                    ...
                    if (testType === 'compete'){ // Test the block competition
                        ...
                        setTimeout(async () => {
                            ...
                            setInterval(async () => { // Sends batchTxn every 5 seconds.
                                const batchTxnData1 = [
                            ...
                            setInterval(async () => { // Sends bulkload every 10 seconds.
