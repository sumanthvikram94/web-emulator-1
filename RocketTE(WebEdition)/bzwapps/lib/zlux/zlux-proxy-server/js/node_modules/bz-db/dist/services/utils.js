...
class Utils {
     ...
     */
    static sumArray(arr, propertyName) {
        const sum = arr.map((bc) => bc[propertyName]).reduce((total, val) => {
            return total + val;
        });
        return sum;
    }
     ...
     */
    static cartesian(arrs) {
        const f = (a, b) => [].concat(...a.map((d) => b.map((e) => [].concat(d, e))));
        const cartesianFun = (a, b, ...c) => (b ? cartesianFun(f(a, b), ...c) : a);
    ...
    static async wait(callback, interval = 100, count = 50) {
        while (count-- > 0) {
            if (await callback()) {
                return true;
            }
    ...
    static getServerNameFromUrl(url) {
        if (url) {
    ...
    static getParamObject(message, detail, type, remotePeerId = '', metadata, dataEntity = '', localLastBlockTime = '', peerLastBlockTime = '') {
        return {
            message,
            detail,
            type,
    ...
    static transferTimestampToDateTime(timestamp) {
    ...
    static transferTimestampToDate(timestamp) {
        let date = new Date();
        if (timestamp && Number(timestamp)) {
    ...
    static date2DateString(date, dateOnly = false, timeOnly = false) {
