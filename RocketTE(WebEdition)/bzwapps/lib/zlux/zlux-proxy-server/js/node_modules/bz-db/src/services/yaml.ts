import { parseDocument, stringify, Node, parse } from 'yaml'
import _ from 'lodash'

function getKeysTree(obj: any, prefix: (string | number)[] = []): (string | number)[][] {
    let keysArray: (string | number)[][] = [];
...
class Yaml {
    ...
    yamlStrToJson(str: string) {
        ...
        if (json && typeof(json) === 'object') {
            ...
            jsonkeys.forEach((key) => {
                const node = doc.getIn(key, true) as Node | undefined
                ...
                if (comments && Array.isArray(comments) && comments.length > 0) {
                    comments.forEach((cmt) => {
                        if (cmt.key === '') {
                            ...
                            const node = doc.getIn(cmt.key, true) as Node | undefined 
                            if (node) {
     ...
     * @returns 
     */
    yamlStringify(obj: object): string {
        return stringify(obj)
    }
}
