...
class MessageSenderWorker {
    ...
    private async ensureMuxerStream(){
        ...
        if (!this.muxerStreamId){ // first time to send message to the channel
            ...
            if (conn){ // the connection id found
                const csarr = conn.streams? conn.streams.filter((str:any)=>{return str.id === this.muxerStreamId}): false;
    ...
    private async createMuxerStream(): Promise<any> {
    ...
    private createInputStream(): PassThrough{
    ...
    public getInputStream(): PassThrough{
    ...
    private async reCreateInputStream(){
    ...
    public async sendString(msg:string){
        if (msg.length > 0){
    ...
    // public async sendBuffer(msg:Buffer){
    //     if (msg.length > 0){
    //         const inStream = await this.getInputStream()
    //         inStream.write(msg)
    //     }
    // }

    // public async sendArray(array: Array<any>){
    //     if (array.length > 0){
    //         const inStream = await this.getInputStream()
    //         array.forEach(ele => {
    //             inStream.write(ele)
...
class MessageSender{
    ...
    private getWorkerId(peerId: string, channel: string): string{
        return peerId + '|' + channel
    }

    public async ensureSender(peerInfo: IPeerMeta, channel: string){
    ...
    public async getInputStream(peerInfo: IPeerMeta, channel: string): Promise<PassThrough> {
        const sendWorker = await this.ensureSender(peerInfo, channel);
        return sendWorker.getInputStream();
    }

    public async send(peerInfo: IPeerMeta, channel: string, msg: string){
        const sendWorker = await this.ensureSender(peerInfo, channel)
        sendWorker.sendString(msg)
    }

    public printSenders(){ // For debug purpose only
        this.workers.forEach((worker) => {
            const activeConn = this.localNode.getConnections(worker.peerInst.peerIdInst)
            const csarr = (activeConn && activeConn.length > 0 && activeConn[0].streams) ? activeConn[0].streams.filter((str:any)=>{return str.id === worker.muxerStreamId}): [];
