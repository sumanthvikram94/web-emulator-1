...
import { DatabaseMetadata, DataEntityMetadata, PersistMethod, PersistType, SyncMode } from './metadata.js';
...
import { Utils, Status, uid, elu, enableElu, yaml, loggers, LoggerConfigurer, Logger, timeTracer } from '../services/index.js';
...
import { NodeMetadata } from '../distribution/node-metadata.js';
import { TransactionError } from '../blockchain/error.js';
import { FileSync } from './file-sync.js'; // BZ-15355, sync file for cluster
...
class DbWorker {
    ...
    async start(dbMeta) {
        try {
            ...
            this.getStatusObservable().subscribe((status) => {
    ...
    addNetMeta(dbMeta) {
        ...
        if (dbMeta.getMetaData().isSingleton === undefined || dbMeta.getMetaData().isSingleton === false) {
            dbMeta.getDataEntities().forEach((de) => {
                ...
                if (this._metaNames.includes(deName) || de.persistMethod === PersistMethod.PERSIST_METHOD_ONLY_MEMORY)
                    return; // create the block de only there is block for the de
    ...
    async loadExtendEntity() {
        const entendEntity = new DataEntityMetadata(CONFIG.EXTEND_ENTITY_META);
        //this._metaNames.push(CONFIG.EXTEND_ENTITY)
        const meta = this._dbMeta.getMetaData().dataEntities.filter((obj) => { return obj.name === entendEntity.name; })[0];
        if (!meta) { //not exist
        ...
        if (entityMetas.rowCount > 0 && entityMetas.data.length > 0) {
            for await (let entity of entityMetas.data) {
                ...
                if (dMetaData) {
                    if (this._metaNames.includes(dMetaData.name) || dMetaData.persistMethod === PersistMethod.PERSIST_METHOD_ONLY_MEMORY)
                        return; // create the block de only there is block for the de
                    const meta = this._dbMeta.getMetaData().dataEntities.filter((obj) => { return obj.name === dMetaData.name; })[0];
                    if (!meta) { //not exist
    ...
    async _setBlockEntity(dMetaData) {
        ...
        const meta = this._dbMeta.getMetaData().dataEntities.filter((obj) => { return obj.name === blockDENameMeta.name; })[0];
        if (!meta) { //add block
    ...
    async loadChain(entityName) {
        const name = entityName;
        const deMeta = this._dbMeta.getDataEntities().get(name) || {};
        const inMemory = deMeta.persistMethod === PersistMethod.PERSIST_METHOD_ONLY_MEMORY;
        ...
        if (!this._metaNames.includes(name) && !inMemory && !inLocal) {
            ...
            const selectOptions = { orderBy: { fields: [(row) => { return _.parseInt(row.index, 10); }] } };
     ...
     * @returns
     */
    async replaceChains(chains, taskId) {
        ...
        for await (let subject of Array.from(chainsData.keys())) {
            const obj = chainsData.get(subject);
     ...
     * @param chains
     */
    async appendChains(peer, chains, txnId) {
        txnLogger.logDebug('Appending chains', txnId);
        const chainsData = Utils.arrayToMap(chains); // it's Map<string, any[]>
    ...
    async execFileTransferForBlock(blocks, subject) {
     ...
     * @param chains
     */
    async appendBlocks(blocks, txnId) {
     ...
     * @param data
     */
    async replaceData(peer, data, taskId) {
        if (!data) {
            throw Error('Empty data to repace');
        }
        data.forEach(async (de) => {
            ...
            if ((rowCount > 0 || partitionCount > 0) && !inMemory && !inLocal) {
                ...
                const fsiMsgArr = this.checkFileSync4BulkLoad(/*peer,*/ dataEntityName, data); // BZ-15355, sync file for cluster
     ...
     * @returns
     */
    async doGetChainsStat(peerChainsStat, peerId, taskId) {
        const stats = new Map();
     ...
     * @returns
     */
    async getStatistics(type = 'both', taskId, peerChainsStat, peerId) {
     ...
     * @returns
     */
    async doGetChainsAndDBStat(peerChainsStat, peerId, taskId) {
    ...
    isTxnApproved(txnp) {
        let isApproved = true;
        const approvals = txnp.getProperty('approvals');
        if (approvals) {
    ...
    blocksNotRejected(blocks) {
        for (const b of blocks) {
            if (b.isConflictWithApproved)
                return false;
        }
        return true;
    }
    clearPendingBlockForBatchTxn(blocks) {
    ...
    async createBatchTxnProcess(blocks, txnId) {
        let payload = [];
        const tempBlockList = [];
        ...
        txnp.once('finally', () => {
            if (CONFIG.TASK_TRACK_PERF === true) {
                const inspected = util.inspect(this._workflow.getPerfRecords());
     ...
     * @returns
     */
    async createTxnProcess(block, chain, txnId, neverGiveUp = false) {
        const payload = {
    ...
    isWeekIntegraty(dataEntityName) {
        const deMeta = this._dbMeta.getDataEntities().get(dataEntityName);
     ...
     * @returns
     */
    async handleTxn(dataEntityName, action, value, options) {
    ...
    async appendLostBlocks(chain, block, txnId) {
    ...
    async validateBlock(chain, block, txnId) {
     ...
     * @returns
     */
    async appendBlock(chain, block, isFromPeer = false, txnId) {
    ...
    * @param dataArr
    */
    async sendFileSyncMsg(peer, fsiMsgArr) {
     ...
     * @param dataArr
     */
    performFileSync(/*peer: NodeInfo,*/ dataArr, isBulkLoad = false) {
        const result = [];
        for (const data of dataArr) {
            ...
            try {
                if (!fsi.removed) {
                    ...
                    const fsm = this._fileSync.getFileSyncMsg4SendFile(fsi, isBulkLoad);
                    result.push(fsm);
     ...
     * @param block: block
     */
    checkFileSync4Block(/*peer: NodeInfo,*/ subject, block) {
     ...
     * @param dataArr
     */
    checkFileSync4BulkLoad(/*peer: NodeInfo,*/ subject, dataArr) {
        if (!this.isSupportFileSync(subject)) {
            return [];
        }
     ...
     */
    async testSyncArbitraryFile(filePath, subject, isAdd) {
        ...
        if (stat.isFile()) {
            const fsi = this._fileSync.getFileSyncInfo(filePath, isAdd);
            if (!isAdd) {
                this._fileSync.deleteFile(fsi);
            }
            ...
            for (const file of files) {
                ...
                if (fse.lstatSync(fp).isFile()) {
                    const fsi = this._fileSync.getFileSyncInfo(fp, isAdd);
    ...
    injectTimestamp(dataEntityName, value) {
        const deMeta = this._dbMeta.getDataEntities().get(dataEntityName);
     ...
     * @returns
     */
    async insert(dataEntityName, value, isSilent = false, taskId, neverGiveUp = false) {
        this.injectTimestamp(dataEntityName, value);
     ...
     * @returns
     */
    async delete(dataEntityName, filter, constraints, isSilent = false, taskId) {
        const txnId = taskId ? taskId : this.genTaskId();
        const options = {
            filter,
            constraints,
            ...
            try {
                ...
                checkTask.on('subTasksClose', async () => {
                    ...
                    const self = {
                        ...
                        serverName: (peers.data.find((d) => d.id === id) || {}).serverURL,
     ...
     * @returns
     */
    createTxn(subject, type, data, options) {
    ...
    async getDbSnapshot() {
        ...
        for (let name of entityNames) {
            if (!this._metaNames.includes(name)) {
                const metadata = this._dbMeta.getMetaData().dataEntities.filter((obj) => { return obj.name === name; })[0];
                let records;
                if (metadata) {
    ...
    async checkStatus() {
        try {
            ...
            checkTask.on('subTasksClose', async () => {
                ...
                const peerIds = [];
                peers.forEach((p) => { peerIds.push(p.id); });
                const dataIds = [];
                const metaDataConflict = await this.select('data_conflict', { status: CONFLICT_STATUS.OPEN }, { selectCluster: true });
                const dataConflictMap = {};
                const dataConflictDetails = [];
                metaDataConflict.data.forEach((item) => {
                ...
                data.push({
                    ...
                    isLocal: true,
                    correctId,
                    listening: this._pNet._node.isListening(),
    ...
    async pushToPeers(entities, checkInFlag, toPeerIds) {
        const taskId = this.genTaskIdShort();
    ...
    async resolvePeers(correctPeerid) {
    ...
    isLocalUpdate(dataEntityName, action = '') {
        if (this._dbMeta.cluster.enabled === false) {
            return true;
        }
        const deMeta = this._dbMeta.getDataEntities().get(dataEntityName) || {};
    ...
    selectInMemory(dataEntityName, action = '') {
        const deMeta = this._dbMeta.getDataEntities().get(dataEntityName) || {};
        const inMemory = deMeta.persistMethod === PersistMethod.PERSIST_METHOD_ONLY_MEMORY;
    ...
    async doCreateEntity(type, dMetaData) {
        const txnId = this.genTaskId();
        ...
        if (aMeta) {
            if (!aMeta.owner)
                dMetaData.owner = this._pNet._node._meta.id; //set the owner when create table if no owner.
    ...
    async createEntities(dMetaDatas) {
        let promissAll = [];
        let pushEntitiesName = [];
        if (Array.isArray(dMetaDatas)) {
            for await (let dMetaData of dMetaDatas) {
                promissAll.push(await this.doCreateEntity("ENTITY" /* SchemaType.PERSIST_ENTITY */, dMetaData));
            }
        }
    ...
    async drop(type, dataEntityName) {
        if (this._pNet.isInCluster()) { //
            //await this.handleTxn(CONFIG.EXTEND_ENTITY, ACTION.DB_CREATE,dMetaData)
            ...
            if (rtn.status) {
                //entity
                const ind1 = this._dbMeta.getMetaData().dataEntities.findIndex((obj) => { return obj.name === dataEntityName; });
                if (ind1 > -1) { //not exist
                ...
                const ind3 = this._metaNames.findIndex((obj) => { return obj === blockDEName; });
                if (ind3 > -1) {
                    this._metaNames.splice(ind3, 1);
                }
                const ind4 = this._dbMeta.getMetaData().dataEntities.findIndex((obj) => { return obj.name === blockDEName; });
                if (ind4 > -1) { //not exist
...
async function startWorker(dbMeta) {
    // if (!worker) {
    //     worker = new DbWorker()
    // }
...
class TaskSender {
    ...
    async sendEvent(data, sendFlag = true) {
        await this.waitForObserverSetting();
        const id = await this.worker.getUID();
