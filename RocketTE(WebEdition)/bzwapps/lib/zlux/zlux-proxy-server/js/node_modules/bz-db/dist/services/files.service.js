...
class FileService {
    ...
    setFileStat(fileName, stat) {
        this._fileStats.set(fileName, stat);
    }
    removeFileStat(fileName) {
    ...
    existFileSync(fileName) {
        const fileName1 = path.resolve(fileName);
        return fse.existsSync(fileName1);
    }
    async readJSON(fileName, encoding = 'utf8') {
        const filePath = path.resolve(fileName);
        if (!fse.existsSync(filePath)) {
            return {};
        }
        let dataText = await this.readFilePromise(filePath, encoding);
        try {
        ...
        if (files.length > 0) {
            for (let i = 0; i < files.length; i++) {
                try {
                    ...
                    let dataText = await this.readFilePromise(fileName, 'utf8');
                    let dataObj;
                    try {
    ...
    async getDirFileSize(dir, subName) {
    ...
    async getDirFileContents(dir, subName) {
        let files = await this.getFilesAsync(dir);
        if (!files || files.length === 0) {
            return [];
        }
        return new Promise(async (resolve) => {
            ...
            for await (const fileName of files) {
                ...
                const content = await this.readFileAsync(fullPath, 'utf8');
                result.push({
                    fileName,
                    content
                });
    ...
    async getDataMap(dir, subName, isWatchFile = false) {
        let files = this.getFiles(dir) || [];
        let dataMap = new Map();
        files = files.filter(file => (subName.length && file.indexOf(`${subName}`) > -1));
        if (files.length > 0) {
            for (let i = 0; i < files.length; i++) {
                try {
                    ...
                    //     }
                    // }
                    let dataText = await this.readFilePromise(fileName, 'utf8', isWatchFile);
                    let dataObj;
     ...
     * @returns object[]
     */
    async returnDataWithName(dir, subName, persistType = PersistType.PERSIST_TYPE_RAW, isAllowSubFolder = false) {
        try {
            let files = [];
    ...
    async readFileAsync(path, opts = 'utf8') {
        return new Promise((resolve) => {
            fse.readFile(path, opts, (err, data) => {
                if (err) {
    ...
    readFilePromise(path, opts = 'utf8', isWatchFile = false) {
    ...
    ;
    convertToJsonWithComment(fileAsString) {
        let cleanJSON = "";
        let pos = 0;
        let done = false;
        while (!done) {
    ...
    clearNFSClientCache(dir, callback) {
        try {
            fs.readdir(dir, () => {
                callback();
            });
    ...
    async clearNFSClientCacheAsync(dir) {
        return new Promise((resolve) => {
            try {
                fs.readdir(dir, () => {
                    resolve(true);
