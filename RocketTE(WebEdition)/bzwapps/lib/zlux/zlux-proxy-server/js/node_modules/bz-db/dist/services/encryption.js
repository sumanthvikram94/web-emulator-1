...
class Encryption {
    static encryptWithKeyAndIV(text, key, iv) {
        var cipher = crypto.createCipheriv('AES-256-CBC', key, iv);
        var encrypted = cipher.update(text, 'utf8', 'hex');
        encrypted += cipher.final('hex');
        return encrypted;
    }
    static decryptWithKeyAndIV(text, key, iv) {
        const cipher = crypto.createDecipheriv('AES-256-CBC', key, iv);
        let decrypted = cipher.update(text, 'hex', 'utf8');
        decrypted += cipher.final('utf8');
        return decrypted;
    }
    static getKeyFromValue(text, salt, length, callback) {
        const rounds = 500;
    ...
    static getKeyFromValueSync(value, salt, length) {
        const rounds = 500;
        return crypto.pbkdf2Sync(value, salt, rounds, length, 'sha256');
    }
    static generateSaltIv(value) {
    ...
    static encryptWithKeyConstIV(text, akey) {
    ...
    static decryptWithKeyConstIV(text, akey) {
        try {
    ...
    static encrypt(encryptAttrs, data) {
        return new Promise((resolve, reject) => {
            try {
                if (!encryptAttrs || encryptAttrs.length === 0 || data.salt) {
                    ...
                    if (count > 0) {
                        ...
                        if (loopNum > 0) {
                            ...
                            for (const _encryptAttr of _encryptAttrs) {
                                this.getKeyFromValue(_encryptAttr, saltIvObj.salt, 32, (key) => {
                                    if (key) {
    ...
    //   });
    // }
    static decrypt(encryptAttrs, data) {
    ...
    //   }
    // }
    static _decrypt_(encryptAttrs, data) {
        try {
            if (data.salt) {
                let salt = this.decryptWithKeyAndIV(data.salt, this.rKey, this.rIV);
                let _encryptAttrs = [...encryptAttrs];
...
Encryption.rKey = Buffer.from([145, 116, 54, 181, 215, 23, 211, 238, 143, 184, 191, 134, 142, 153, 69, 143, 192, 179, 40, 219, 10, 83, 145, 72, 199, 67, 146, 248, 2, 245, 190, 113]);
Encryption.rIV = Buffer.from([145, 116, 54, 181, 215, 23, 211, 238, 143, 184, 191, 134, 142, 153, 69, 143]);
Encryption.rString = 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890';
export { Encryption };
