...
class BlockChain {
    ...
    constructor(data, txnRedundancyFactor = 0) {
    ...
    set txn_redundancy_factor(length) {
    ...
    generateBlockCandidate(txns, createdBy, batchts, competeFactor) {
    ...
    getBlockByHash(hash) {
        const bc = prvProps.get(this);
        let blocks = bc.chain.filter((block) => block.hash === hash);
    ...
    isValid() {
        ...
        for (let i = 1; i < chain.length; ++i) {
            /* eslint-disable security/detect-object-injection */
            const currentBlock = chain[i], prevBlock = chain[i - 1], pad = '0'.repeat(bc.difficulty);
            const incorrectPadding = (!currentBlock.hash.startsWith(pad) || !prevBlock.hash.startsWith(pad)), incorrectHash = currentBlock.hash !== currentBlock.calculateHash(), incorrectFollow = currentBlock.prevHash !== prevBlock.hash;
            /* istanbul ignore next */
            if (incorrectPadding || incorrectHash || incorrectFollow)
                return false; //It should be impossible for this branch to return
        }
        return true;
    }
    /**
     ...
     * @memberof Blockchain
     */
    addBlocks(blocks, keepPendingBlockWithSameIdx = false, txnId) {
        const addedBlocks = [];
        try {
     ...
     * @memberof Blockchain
     */
    addBlock(block, keepPendingBlockWithSameIdx = false, txnId, doWithoutTouch) {
        ...
        if (!block.isValid())
            throw new BlockError(`Invalid block: ${block.toJSON()}`);
    ...
    clearHistory() {
        ...
        if (overflowIdx <= 0)
            return; // Keep at least 100 blocks in chain
    ...
    updateBlockIdx(block, includesHash = true) {
        ...
        if (includesHash)
            block.prevHash = this.getBlock(-1).hash;
     ...
     */
    logInvalidBlock(block, validateResult = {}, txnId) {
     ...
     */
    validateBlockIdx(block, validateResult = {}, txnId) {
     ...
     */
    validateChainLink(block, validateResult = {}, txnId) {
     ...
     */
    blockSelfValidate(block, validateResult = {}, txnId) {
     ...
     * @returns
     */
    validateWithPending(block, validateResult = {}, isApproved = false, peerId, txnId) {
     ...
     */
    validateBlock(block, isApproved = false, peerId, txnId) {
        txnLogger.logDebug('Validating block: ' + JSON.stringify(block.toJSON()), txnId);
        const validateResult = { isValid: true };
