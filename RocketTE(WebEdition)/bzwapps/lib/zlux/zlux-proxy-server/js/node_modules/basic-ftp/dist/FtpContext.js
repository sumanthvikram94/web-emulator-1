...
class FTPContext {
    ...
    constructor(timeout = 0, encoding = "utf8") {
        ...
        this.keepAliveTimerOptions = "0";
        this.keepAliveTimerValue = "1";
        this.portActive = false;
        this.activeServer = undefined;
        this.transferMode = true;
    }
    setKeepAlive(option, timer) {
        this.keepAliveTimerOptions = option;
        this.keepAliveTimerValue = timer;
        if (Number(option)) { //if enabled
            this.keepAliveTimer = setInterval(() => {
                this.send("NOOP");
            }, Number(timer) * 60 * 1000);
        }
    }
    /**
     * Close the context.
    ...
    send(command) {
        ...
        this.log(message);
        if (!this._socket.destroyed) { //stops crash if command is sent (ex:keepalive) right after socket was closed
    ...
    handle(command, responseHandler) {
        ...
        return new Promise((resolvePromise, rejectPromise) => {
            ...
            if (command) {
                ...
                if (this.keepAliveTimer) {
                    clearTimeout(this.keepAliveTimer);
                    this.setKeepAlive(this.keepAliveTimerOptions, this.keepAliveTimerValue);
                }
            }
        });
    }
    ...
    _onControlSocketData(chunk) {
        ...
        for (const message of parsed.messages) {
            ...
            //if keepalive is used and the current msg is 200, dont notify task thats waiting for a response yet
            if ((this.keepAliveTimerOptions == "1") && (code == 200) && ((message.startsWith("200 NOOP") || (message.startsWith("200 OK"))))) {
                //ignore NOOP response. Try not to ignore other 200 responses like TYPE response
            }
            else {
