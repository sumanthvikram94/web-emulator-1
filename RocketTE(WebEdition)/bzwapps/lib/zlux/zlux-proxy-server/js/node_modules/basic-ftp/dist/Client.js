...
//import { uploadFrom, downloadTo, enterPassiveModeIPv6, enterPassiveModeIPv4, UploadCommand, enterActiveModeIPv6, enterActiveModeIPv4 } from "./transfer"
...
class Client {
     ...
     */
    constructor(passive = true, timeout = 30000) {
        /**
        ...
        this.availableListCommand = "LIST";
        this.ftpserverType = 4; //default to Unix
        this.JES = 0;
        this.hasMLSD = false;
        this.ftp = new FtpContext_1.FTPContext(timeout);
        this.ftp.transferMode = passive;
        if (!this.ftp.transferMode) {
            //this.prepareTransfer = this._enterFirstCompatibleMode([enterActiveModeIPv6, enterActiveModeIPv4])
            this.prepareTransfer = this._enterFirstCompatibleMode([transfer_1.enterActiveModeIPv4]);
        }
        else {
     ...
     */
    connect(host = "localhost", port = 21, keepAliveTimerOptions = "0", keepAliveTimerValue = "1") {
        this.ftp.reset();
        this.ftp.setKeepAlive(keepAliveTimerOptions, keepAliveTimerValue);
     ...
     */
    connectImplicitTLS(host = "localhost", port = 21, tlsOptions = {}, keepAliveTimerOptions = "0", keepAliveTimerValue = "1") {
        this.ftp.log("connectImplicitTLS1");
        this.ftp.reset();
        this.ftp.setKeepAlive(keepAliveTimerOptions, keepAliveTimerValue);
        this.ftp.log("connectImplicitTLS2");
        ...
        this.ftp.log("connectImplicitTLS3");
    ...
    _handleConnectResponse() {
        this.ftp.log("_handleConnectResponse");
        return this.ftp.handle(undefined, (res, task) => {
            if (res instanceof Error) {
                this.ftp.log("_handleConnectResponse2");
                this.ftp.socket.destroy();
                task.resolve({ code: 500, message: res.message });
                ...
                this.ftp.log("_handleConnectResponse3");
                task.resolve(res);
            }
            // Reject all other codes, including 120 "Service ready in nnn minutes".
            else {
                this.ftp.log("_handleConnectResponse4");
    ...
    sendIgnoringError(command) {
        return this.ftp.handle(command, (res, task) => {
            if (res instanceof FtpContext_1.FTPError) {
                ...
                if (command == "FEAT") {
                    this.ftp.log("client requested FEAT");
                    const features = new Map();
                    // Not supporting any special features will be reported with a single line.
                    if (parseControlResponse_1.isMultiline(res.message)) {
                        ...
                        this.ftp.log(features);
                        if (features.has('MLSD')) {
                            this.hasMLSD = true;
                            this.availableListCommand = "MLSD";
                        }
                        else
                            this.availableListCommand = "LIST";
                    }
                    ...
                    //determine host type to help with picking list parser
                    this.ftpserverType = 4; //default to Unix
                    let type;
                    type = res.message.substring(4);
                    type = type.trim();
                    let n = type.indexOf(' ');
                    if (n > -1) {
                        type = type.substring(0, n);
                    }
                    if ((equalsIgnoringCase(type, "OS/400")) || (equalsIgnoringCase(type, "z/400")))
                        this.ftpserverType = Consts_1.Consts.FTP_SERVER_TYPE_AS400;
                    else if (equalsIgnoringCase(type, "UNIX"))
                        this.ftpserverType = Consts_1.Consts.FTP_SERVER_TYPE_UNIX;
                    else if (equalsIgnoringCase(type, "Windows_NT"))
                        this.ftpserverType = Consts_1.Consts.FTP_SERVER_TYPE_WINDOWS;
                    else if (equalsIgnoringCase(type, "Windows2000"))
                        this.ftpserverType = Consts_1.Consts.FTP_SERVER_TYPE_WINDOWS;
                    else if ((equalsIgnoringCase(type, "MVS")) || (equalsIgnoringCase(type, "OS/MVS")) || (equalsIgnoringCase(type, "z/MVS")))
                        this.ftpserverType = Consts_1.Consts.FTP_SERVER_TYPE_MVS;
                    else if ((equalsIgnoringCase(type, "VM")) || (equalsIgnoringCase(type, "OS/VM")) || (equalsIgnoringCase(type, "z/VM")))
                        this.ftpserverType = Consts_1.Consts.FTP_SERVER_TYPE_VM;
                    else if ((equalsIgnoringCase(type, "VSE")) || (equalsIgnoringCase(type, "OS/VSE")) || (equalsIgnoringCase(type, "z/VSE")))
                        this.ftpserverType = Consts_1.Consts.FTP_SERVER_TYPE_VSE;
                    else if ((equalsIgnoringCase(type, "VMS")) || (equalsIgnoringCase(type, "OS/VMS")) || (equalsIgnoringCase(type, "z/VMS")))
                        this.ftpserverType = Consts_1.Consts.FTP_SERVER_TYPE_VMS;
                    else if (equalsIgnoringCase(type, "NETWARE"))
                        this.ftpserverType = Consts_1.Consts.FTP_SERVER_TYPE_NETWARE;
                    else if (equalsIgnoringCase(type, "VOS"))
                        this.ftpserverType = Consts_1.Consts.FTP_SERVER_TYPE_VOS;
                    else if (equalsIgnoringCase(type, "Nonstop"))
                        this.ftpserverType = Consts_1.Consts.FTP_SERVER_TYPE_NONSTOP;
                    this.ftp.log("FTP Server Type: " + this.ftpserverType);
                }
                else if (equalsIgnoringCase(command, "SITE FILETYPE=JES"))
                    this.JES = 1;
                else if (equalsIgnoringCase(command, "SITE FILETYPE=SEQ"))
                    this.JES = 0;
                task.resolve(res);
            }
        });
    ...
    async _uploadFromStream(source, remotePath, command) {
        ...
        try {
            ...
            const ret = await this.prepareTransfer(this.ftp);
            if (ret.code >= 400) {
                return { code: ret.code, message: ret.message };
            }
    ...
    async _downloadToStream(destination, remotePath, startAt) {
        ...
        try {
            ...
            //destination.end() //dont kill the stream for other transfers 
        }
    }
    /**
     * List files and directories in the current working directory
     *
     * @param [command]  Custom LIST command
     */
    async list(command = "") {
        //const validPath = await this.protectWhitespace(path)
        let lastError;
        ...
        */
        ...
        try {
            const listcommand = command != "" ? command : this.availableListCommand;
            const parsedList = await this._requestListWithCommand(listcommand);
            //this.availableListCommand = listcommand
            return parsedList;
        }
        catch (err) {
    ...
    async _requestListWithCommand(command) {
        this.ftp.log("_requestListWithCommand");
        const buffer = new StringWriter_1.StringWriter();
        let res = await transfer_1.downloadTo(buffer, {
            ftp: this.ftp,
            tracker: this._progressTracker,
            command,
            remotePath: "",
        ...
        this.ftp.log(text);
        let list = {}; // local variable should be defined with let, not var
        list = res;
        list.data = this.parseList(text, this.ftpserverType, this.JES, this.hasMLSD, this.ftp);
        return list;
    }
    /**
    ...
    /**
     * Download a folder as ZIP file into writable stream
     *
     * @param destination  The destination stream to write the ZIP file
     * @param remoteDirPath  Remote directory to download. Current working directory if not specified.
     */
    async archiveDir(destination, modeOptions, remoteDirPath) {
        return this._exitAtCurrentDirectory(async () => {
            if (remoteDirPath) {
                await this.cd(remoteDirPath);
            }
            ...
            await this._archiveFromWorkingDir(archive, modeOptions); // add files/subfolders into ZIP
     ...
     * @protected
     */
    async _archiveFromWorkingDir(archive, modeOptions, parent) {
        const files = await this.list(); // gets file list of current working dir
        for (const file of files.data) {
            const filename = file.Name.trim();
            if (file.type === 'folder') { // Add sub folder into ZIP
                archive.append(Buffer.from([]), { name: filename + '/', prefix: parent }); // adds the folder into ZIP
                await this.cd(filename);
                await this._archiveFromWorkingDir(archive, modeOptions, parent ? parent + '/' + filename : filename);
                await this.cdup();
            }
            else if (file.type === 'file') { // Add file into ZIP
                const stream = new stream_1.PassThrough(); // it output each data it receives directly
                archive.append(stream, { name: filename, prefix: parent }); // prefix means the subfolder name in ZIP
                const type = this._getAutoType(modeOptions, filename);
                console.log(`[FTP Folder Downloading] ${type}`);
                console.log(`[FTP Folder Downloading] subfile: ${parent ? parent + '/' : ''}${filename}`);
                await this.send(type);
                const result = await this.downloadTo(stream, filename); // retrieve the file from FTP server to stream
                ...
                if (result.code >= 300) {
                    ...
                    this.ftp.log('Finished transfering: ' + filename);
                }
                stream.end(); // When there are multiple files in folder, the archiver will wait for one file finish before start another one.
            }
        }
    }
    _getAutoType(options, fileName) {
        const mode = options['mode'];
        if (!mode)
            return 'TYPE I'; // return default value if autoMode is not provided
        let choices; // Set choices according to autoMode configuration
        if (mode === 'ascii') {
            choices = ['TYPE A', 'TYPE I'];
        }
        else {
            choices = ['TYPE I', 'TYPE A'];
        }
        let idx = 0;
        ...
        if (exceptions && Array.isArray(exceptions)) {
            ...
            for (let i = 0; i < exceptions.length; i++) {
                if (ext === exceptions[i].toUpperCase()) { // If the file extension is in exception list, choose 1, otherwise choose 0
                    idx = 1;
                    break;
                }
            }
    ...
    async _downloadFromWorkingDir(localDirPath) {
        ...
        // Here fixed a bug, although we are not using this function.
        // list() doesn't return array, but it returns an object. 
        // The "data" field of the object is the array needed.
        // Furthermore, the isDirectory and isFile don't exist in the file object, should use "type" instead.
