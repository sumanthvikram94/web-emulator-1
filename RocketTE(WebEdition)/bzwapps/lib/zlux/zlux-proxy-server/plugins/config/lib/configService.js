...
function percentEncode(value){
  ...
  // keep bzadmin adn bzw use the same escape method
...
function replaceOrCreateFile(response, filename, directories, scope, relativePath,
  ...
  var mode = 0644; //TODO is 700 good for us?
  ...
  fs.open(path,'w',mode,function(error, fd) {
    if (!error) {
      var offset = 0;
      // var contentLength = content.length;  // When there is multi-byte charactors, content.length is shorter than buffer length
      var buff = Buffer.from(content,'utf8');
      var contentLength = buff.length;
