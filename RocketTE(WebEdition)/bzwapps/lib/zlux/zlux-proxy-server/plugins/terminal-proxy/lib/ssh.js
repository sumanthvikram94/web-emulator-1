...
//   }catch(err){
//     var swcrypto = false;
//     console.warn('diffie-hellman/browser not installed. Will use crypto of node.js');
//   }
// }
...
if (curve25519Supported) {
  SUPPORTED_KEX.unshift('curve25519-sha256');
  SUPPORTED_KEX.unshift('curve25519-sha256@libssh.org');
}
...
exports.processEncryptedData = function(terminalWebsocketProxy,data,sessId) {
  return ssh.processEncryptedData(terminalWebsocketProxy,data,sessId);
};

exports.sendSSHData = function(terminalWebsocketProxy,data,sessId) {
  return ssh.sendSSHData(terminalWebsocketProxy,data,sessId);
};

exports.reOrderKeyExchangeList= function(keys)  {
  if(Array.isArray(keys) && keys.length>0){
    keys.reverse();
    keys.forEach(key=>{
      const exist=SUPPORTED_KEX.findIndex(e=>{return key===e});
      if(exist>-1){  //provide key has support
        SUPPORTED_KEX.splice(exist,1);//remove from older postion
        SUPPORTED_KEX.unshift(key) ;// insert to top
      } 
    })
  }
};
...
ssh.sendSSHData = function (terminalWebsocketProxy,jsonData,sessId){
   if(!sshLogger) sshLogger = terminalWebsocketProxy.logger;
   const clientIP = terminalWebsocketProxy.clientIP;
   const log_header = `ID=${sessId}, SSH, ClientIP=${clientIP}`
   ...
   var pdu;
   sshLogger.debug(`[${log_header}] send=` + CODE_TO_NAME[msgCode]);
   switch (msgCode) {
     ...
     case SSH_MESSAGE.SSH_MSG_CHANNEL_DATA:{
       ...
       if (channel) {
         ...
         sshLogger.log(sshLogger.FINER, `[${log_header}, MsgSeq: ${sessionData.messagesReceived}] - `
         ...
         sshLogger.warn(`[${log_header}, MsgSeq: ${sessionData.messagesReceived}] - `
     ...
     case SSH_MESSAGE.SSH_MSG_IGNORE: {
      ...
      let msgCodeBuffer = Buffer.alloc(1);
      msgCodeBuffer.writeUInt8(SSH_MESSAGE.SSH_MSG_IGNORE);
      const payload=generateSSHv2PDUBytes([msgCodeBuffer,new VarData()]);
      sshLogger.log(sshLogger.FINER, `[${log_header}, MsgSeq: ${sessionData.messagesReceived}] - ` +"sending SSH_MSG_IGNORE");
      pdu = new SSHv2PDU(SSH_MESSAGE.SSH_MSG_IGNORE,payload);
      break;

    }
     
   }
  sshLogger.debug(`[${log_header}] send=${CODE_TO_NAME[msgCode]} done`);
   if (pdu!=null){
...
// main method. 
ssh.processEncryptedData = function (terminalWebsocketProxy,rawData,sessId){
  if(!sshLogger) sshLogger = terminalWebsocketProxy.logger;
  const clientIP = terminalWebsocketProxy.clientIP;
  const log_header = `ID=${sessId}, SSH, ClientIP=${clientIP}`
  var sshMessages = [];
  ...
  if (!sessionData){
    ...
    sshLogger.log(sshLogger.FINE, `[${log_header}] - serverVersion: ${serverVersion}`);
    return sshMessages;
  }
  ...
  if (sessionData.incompletePacketBuffer){
    sshLogger.log(sshLogger.FINE, `[${log_header}, MsgSeq: ${sessionData.messagesReceived}] - ` 
  ...
  while (currentPosition<rawDataLength){ 
    ...
    var readObject = readSSHv2PDUData(sessionData,rawData,currentPosition, log_header); // JERRY: Payload is stored in readObject.sshv2PDU.data
    ...
    if (readObject.Continue){  // JERRY: Incomplete packet, continue receiving next part of data.
      ...
      sshLogger.log(sshLogger.FINEST, `[${log_header}, MsgSeq: ${sessionData.messagesReceived}] - `
    ...
    sshLogger.debug(`[${log_header}] rcv=` + CODE_TO_NAME[msgCode]);
    switch (msgCode) {
      ...
      case SSH_MESSAGE.SSH_MSG_DISCONNECT:{
        ...
        sshLogger.log(sshLogger.INFO, `[${log_header}, MsgSeq: ${sessionData.messagesReceived}] - `
      ...
      case SSH_MESSAGE.SSH_MSG_CHANNEL_REQUEST: {
        ...
        let payload = '';
        let replyMsgCode = SSH_MESSAGE.SSH_MSG_CHANNEL_FAILURE;
        const peerChannelNumber = new Number(sshMessage.recipientChannel)
        const requestName = sshMessage.requestName? sshMessage.requestName.toString(): ''
        
        /**
         * TBD, handle each kind of request. Refer to putty source code.
         */
        switch (requestName) {
          case 'keepalive@openssh.com': // Server initicated keep-alive. Reply SSH_MSG_CHANNEL_FAILURE is fine.
            sshLogger.log(sshLogger.FINE, `[${log_header}, MsgSeq: ${sessionData.messagesReceived}] - keep alive received`);
            break;
          ...
          case 'window-change':
          default:
            sshLogger.warn(`[${log_header}, MsgSeq: ${sessionData.messagesReceived}] - Unhandled channel request received.\n` + 
              `Request name: ${requestName}\nData = ${hexDump(sshv2PDU.data)}`);
        }

        if (sshMessage.needsReply === 1){ // reply is required
          let msgCodeBuffer = Buffer.alloc(1);
          msgCodeBuffer.writeUInt8(replyMsgCode);
          const sessionVar = new VarData(Buffer.from(payload));
          const openChannelRequestData = [msgCodeBuffer,sessionVar, peerChannelNumber];
          const openChannelRequestPDU = new SSHv2PDU(replyMsgCode, generateSSHv2PDUBytes(openChannelRequestData));
          writeSSHv2PDU(terminalWebsocketProxy.netSend.bind(terminalWebsocketProxy), sessionData, openChannelRequestPDU); // Reply the request to server
        }
      ...
      case SSH_MESSAGE.SSH_MSG_KEXINIT:{
        ...
        sshLogger.log(sshLogger.INFO,`[${log_header}] Host support keyExchangeAlgorithms:`+keyExchangeAlgorithms.toString());
        ...
        sshLogger.log(sshLogger.INFO,`[${log_header}] ` + "chosen keyExchangeAlgorithms:"+sessionData.keyExchangeAlgorithms[0]);
        switch (sessionData.keyExchangeAlgorithms[0]) {
          case 'ecdh-sha2-nistp256':
            hashAlgorithem = 'sha256';
            dh=crypto.createECDH('prime256v1');
            break;  
          case 'ecdh-sha2-nistp384':
            hashAlgorithem = 'sha384';
            dh=crypto.createECDH('secp384r1');
            break;  
        ...
        if (dh){ 
          if(dh.type==="unNative"){
            dh.generateKeys();     
            publicKeys = dh.getPublicKey();
            publicKeys=dh.convertPublicKey(publicKeys)
          }else{
            dh.generateKeys();     
            publicKeys = dh.getPublicKey();
            publicKeys=KeyExchangeUtil.convertPublicKey(publicKeys)
            var idx = 0;
          ...
          if (traceCrypto) console.log("----------publicKeys takes(ms): "+(Date.now()-startTime))
      ...
      case SSH_MESSAGE.SSH_MSG_KEX_DH_GEX_GROUP:{
        if (sessionData.expectedReplyMsgCode){
          if (sessionData.expectedReplyMsgCode!=msgCode){
            //DISCONNECT_ERROR
            sshLogger.warn(`[${log_header}] - Disconnect_error`);
        ...
        if (sessionData.sentMsgCode==SSH_MESSAGE.SSH_MSG_KEX_DH_GEX_REQUEST){
          ...
          sshLogger.debug(`[${log_header}] sessionData.generator`);

          if (traceCrypto) console.log("-----DH Group publicKeys takes(ms) "+(Date.now()-startTime))

          // var dh = nodeVersion >= 12 && swcrypto
          ...
          var dh = crypto.createDiffieHellman(sessionData.prime,sessionData.generator); // For node.js V16, the native DiffieHellman's performance is as good as the DiffieHellman/browser package.

	        sshLogger.debug(`[${log_header}] made dh`);
          ...
          dh.generateKeys();
          sshLogger.debug(`[${log_header}] made keys`);
      ...
      case SSH_MESSAGE.SSH_MSG_KEX_DH_GEX_REPLY: {
        ...
        if(sessionData.dh.type==="unNative"){
          sessionData.f=sessionData.dh.convertPublicKey(sessionData.f)
        }else{
          sessionData.f=KeyExchangeUtil.convertPublicKey(sessionData.f)
        }
        ...
        if (sessionData.dh){
          const startTime=Date.now()
          sharedSecret=sessionData.dh.computeSecret(sessionData.f);
          sharedSecret=KeyExchangeUtil.convertToMpint(sharedSecret);
          if (traceCrypto) console.log("-----sharedSecret takes(ms): "+(Date.now()-startTime))
          if (traceCrypto) console.log("sharedSecret\n",sharedSecret.join(","))
          if(sharedSecret){
            sessionData.k=sharedSecret;
          }
        }
        // } else if (sessionData.dhMeta !== undefined) { // So it's using the diffie-dellman in thread pool
        //   const taskData = { // Creates a diffie-hellman computeSecret task
        //     type: 'df-computeSecret',
        //     payload: {
        //       dhMeta: sessionData.dhMeta,
        //       pubkey: sessionData.f
        //     }
        //   }
        //   const sharedSecret = Buffer.from(await workerPool.runTaskAsync(taskData)); // Runs the task in thread pool
        //   sharedSecret=KeyExchangeUtil.convertToMpint(sharedSecret);
        //   if(sharedSecret){
        //     sessionData.k=sharedSecret;
        //   }
        ...
        // }else{
          sessionData.e.copy(exchangeDataBuffer, cp, 0); 
          cp += clientPubKeyLength;
        //}  
        ...
        // if(sessionData.dh.type!=="unNative" && 1!=0){
        //   if (sessionData.f[fIndex] & 0x80)
        //   exchangeDataBuffer[cp++] = 0;
        ...
        // }else{
          sessionData.f.copy(exchangeDataBuffer, cp, 0); 
          cp += serverPubKeyLength;
        //}
        ...
        // }else{
          sessionData.k.copy(exchangeDataBuffer, cp, 0); 
       // }
      ...
      case SSH_MESSAGE.SSH_MSG_CHANNEL_CLOSE: {
        sessionData.sshMessageCode = SSH_MESSAGE.SSH_MSG_CHANNEL_CLOSE;
        sshMessages.push(sshMessage);
        break;
      }      
      default :{
    ...
    sshLogger.debug(`[${log_header}] rcv=${CODE_TO_NAME[msgCode]} done`);
    terminalWebsocketProxy.sshSessionData=sessionData;
  }
...
function readSSHv2PDUData(sessionData,rawData,currentPosition,log_header){
  ...
  sshLogger.log(sshLogger.FINEST, `[${log_header}, MsgSeq: ${sessionData.messagesReceived}] - Decrypted payload: \n${hexDump(payload)}`); 
  ...
  if (sessionData.readingMACAlgorithm) {
    ...
    if (computedMac == undefined) {
      sshLogger.warn(`[${log_header}] - mac verification failed - unable to get computed version`);
    } else {
      if (receivedMac.toString('hex')!=computedMac.toString('hex')){
        sshLogger.warn(`${log_header}, MsgSeq: ${sessionData.messagesReceived}] - MAC verification failed - computed does not match received`); 
        sshLogger.warn(`${log_header}, MsgSeq: ${sessionData.messagesReceived}] - received MAC: ${receivedMac.toString('hex')}`); 
        sshLogger.warn(`${log_header}, MsgSeq: ${sessionData.messagesReceived}] - computed MAC: ${computedMac.toString('hex')}`); 
      }
      if(traceCrypto) {
  ...
  sshLogger.log(sshLogger.FINEST, `[${log_header}, MsgSeq: ${sessionData.messagesReceived}] - PDU read complete`); 
...
class KeyExchangeUtil{
  constructor() {}
  static convertPublicKey(key) {
    let newKey;
    let idx = 0;
    let len = key.length;
    while (key[idx] === 0x00) {
...
class Curve25519Exchange {
  constructor(){
    this.Algorithms = '25519';
    this.type="unNative";
    this.keyExchangeAlgorithms="curve25519-sha256@libssh.org";
    this._keys = null;
  }
  generateKeys(){
    if (!this._keys)
