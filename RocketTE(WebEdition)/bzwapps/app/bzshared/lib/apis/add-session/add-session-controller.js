...
class AddSessionController {
    ...
    async prepSession(name, req) {
        const instanceDir = this.context.plugin.server.config.user.instanceDir;
        ...
        return new Promise((resolve, reject) => {
            fs.readFile(path.resolve(instanceDir + subPath), (err, rep) => {
                if (err) reject(err);
                try {
                    ...
                    let session = {
                        "session": {
                            ...
                            'is3270Session': data.type !== p3270 && data.type.indexOf('3270') > -1,
                            'is3270pSession': data.type === p3270,
                            'isVTSession': data.type.indexOf('VT') > -1,
                            'is5250Session': data.type !== p5250 && data.type.indexOf('5250') > -1,
